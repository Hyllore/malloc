...
def page_reclaims(prog):
    ...
    pipe = cmd.Popen(com.split(), stdout=cmd.PIPE, stderr=cmd.PIPE, shell=True)
...
def cmd_output(com):
    pipe = cmd.Popen(com, stdout=cmd.PIPE, stderr=cmd.PIPE, shell=True)
    output, errput = pipe.communicate()
